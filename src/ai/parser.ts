...
const SYSTEM_PROMPT =
  ...
  `3. Extract: activity type, duration in days (7–60), and required proof count.\n` +
  ...
  `9. Cap durationDays at 60. If the user says more than 60 days, set durationDays = 60.\n` +
...
export async function parseCommitment(text: string): Promise<ParseResult> {
  try {
    ...
    if (
      ...
      isNaN(durationDays) || durationDays < 7 || durationDays > 60 ||
